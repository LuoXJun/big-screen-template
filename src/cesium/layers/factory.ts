...
export type LayerFactory<C extends LayerConfig = LayerConfig> = (
    name: string,
    config?: C
) => MapLayer<unknown>;
 ...
 */
export function registerLayerFactory<C extends LayerConfig>(
    type: LayerType,
    factory: LayerFactory<C>
): void {
    factories.set(type, factory as LayerFactory<never>);
}

/** 按类型创建图层实例（未注册的类型抛出错误） */
export function createLayer<T = unknown>(
    type: LayerType,
    name: string,
