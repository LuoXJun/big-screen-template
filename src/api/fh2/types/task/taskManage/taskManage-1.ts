 ...
 */
/** 创建飞行任务 */
export type createFlightTasksData = {
    /** 目标飞行任务信息 */
    task?: {
        /** 任务 UUID */
        uuid?: string;
...
/** 创建飞行任务 请求体 */
export type createFlightTasksBody = {
    /** 任务类型:1=立即任务 / 2=重复定时任务 / 3=单次定时任务 / 5=连续执行任务。 注:类型 4 / 6 由系统自动创建，调用方不可传入;类型 7(试飞任务)请使用专属端点 POST /flight-tasks/test-fligh */
    task_type: 1 | 2 | 3 | 5;
...
/** 创建飞行任务（高级版，返回子任务） */
export type createAdvancedData = {
    /** 目标飞行任务信息 */
    task?: {
        /** 任务 UUID */
        uuid?: string;
    ...
    sub_task?: Array<{
        /** 子任务 UUID */
        uuid?: string;
        /** 子任务开始时间，单位：秒（unix 时间戳） */
        begin_at?: number;
        /** 子任务结束时间，单位：秒（unix 时间戳） */
        end_at?: number;
    }>;
};
/** 创建飞行任务（高级版，返回子任务） 请求体 */
export type createAdvancedBody = {
    /** 任务类型:1=立即任务 / 2=重复定时任务 / 3=单次定时任务 / 5=连续执行任务。注:类型 4/6 由系统自动创建，调用方不可传入,7 走专属端点 */
    task_type: 1 | 2 | 3 | 5;
    /** 重复频率类型（task_type=2 或 5 时必填）：0-不重复 / 1-每几秒 / 2-每几分钟 / 3-每几小时 / 4-每几天 / 5-每几周 / 6-每几月（按日期）/ 7-每几月（按星期）/ 8-绝对每年 */
    repeat_type?: 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8;
    /** 间隔（每 N 个单位执行一次），单位由 repeat_type 决定。task_type=2 或 5 时必填 */
